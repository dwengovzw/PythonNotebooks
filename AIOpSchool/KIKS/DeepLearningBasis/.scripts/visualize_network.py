...
def resize_keep_ratio(im, height):
    ...
    new_im = im.resize((new_width, height), PIL.Image.Resampling.LANCZOS)
    return new_im
...
def get_network_image(base='conv_base', n_conv_layers=1, dropout=False, n_ff_layers=1, n_ff_input=16):
    ...
    for layer in layers:
        ...
        if layer['name'] == 'conv':
            ...
            ff_layer_im_local = ff_layer_im_local.resize((ff_layer_im_local.size[0], int(ff_layer_im_local.size[1] / 2)), PIL.Image.Resampling.LANCZOS)
