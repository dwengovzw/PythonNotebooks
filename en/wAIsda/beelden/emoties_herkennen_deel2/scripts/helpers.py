...
def lees_dataset():
    afbeeldingen_blij, labels_blij = laadt_bestanden_in_map_met_label("dataset/blij", label="blij")
    afbeeldingen_verbaasd, labels_verbaasd = laadt_bestanden_in_map_met_label("dataset/verbaasd", label="verbaasd")
    ...
    labels_one_hot = one_hot_encode_labels(labels, ["blij", "verbaasd"])
    ...
    train_afbeeldingen, test_afbeeldingen, train_labels, test_labels = train_test_split(overige_afbeeldingen, overige_labels, test_size=0.2)
    return train_afbeeldingen, train_labels, test_afbeeldingen, test_labels, validatie_afbeeldingen, validatie_labels

def evalueer_model(model, test_afbeeldingen, test_labels):
    ...
    confusion_matrix = create_confusion_matrix_for_one_hot_encoded_labels(test_labels, predictions, ["blij", "verbaasd"])
    visualize_confussion_matrix_in_heatmap(confusion_matrix, ["blij", "verbaasd"])
